...
- **信号源**：OKX USDT-SWAP 永续合约 5 分钟 K 线，检测 MA13/MA35 均线斜率的「拐点」——单根斜率进入近零带、且此前有真实单边趋势（沿用斜率归零监控系统的 `check_turn_point` 判定）。
  - `bottom`（见底回升，偏多）→ 开多
  - `top`（见顶回落，偏空）→ 开空
- **趋势过滤**：入场前看大级别（默认 1h）MA35 斜率方向，**只做顺趋势**——1h 向上只开多、向下只开空，过滤掉逆势假信号（`TREND_FILTER` / `TREND_BAR` / `TREND_THRESHOLD`）。
- **离场（三层）**：
  1. **硬止损**：开仓即挂交易所端 `--slTriggerPx`（`SL_PCT`，默认 2%），程序崩了也生效。
  2. **追踪止损**：持仓期间记录极值，从极值回撤 `TRAIL_PCT`（默认 1.5%）离场——让利润奔跑，不再死等反向拐点。
  3. **反向拐点**：见顶回落平多 / 见底回升平空（兜底）。
  - 程序侧另做轮询兜底，实时价（ticker）越界即平。
...
| `TRAIL_PCT` | `0.015` | 追踪止损回撤百分比（1.5%，从极值回撤此幅度离场） |
| `REVERSE_ON_SIGNAL` | `false` | 反向信号是否反手（默认只平仓） |
| `MAX_OPEN_POSITIONS` | `3` | 同时最大持仓合约数 |
| `MAX_TOTAL_LOSS` | `300` | 总亏损熔断阈值（USDT） |
| `TREND_FILTER` | `true` | 是否启用大级别趋势过滤（只做顺趋势） |
| `TREND_BAR` | `1H` | 趋势过滤用的大级别周期 |
| `TREND_THRESHOLD` | `0.02` | 大级别 MA35 斜率阈值（%），\|斜率\|>此值才算有趋势 |
